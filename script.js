...
// Save input City to local storage and push to search history
    ...
    // get item from storage and append it to the screen
     var savedSearch = JSON.parse(localStorage.getItem("User"))
        if(savedSearch !== null) {
            for (var i = 0; i<savedSearch.length; i++) {
                $(".searchhistory").html("<button type='button' class='btn btn-primary'>" + savedSearch[i] + "</button>")
        }};
...
$('#submit').click (function() {
    ...
    // Save the City Name to Local Storage
        ...
        $(".searchhistory").html("<button type='button' class='btn btn-primary'>" + searchElem + "</button>")
